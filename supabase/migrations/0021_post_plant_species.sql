-- Half B: a plant tag can name a specific species (WCVP binomial), not just a
-- genus. We keep the (post_id, genus) primary key (plant_confirm FKs it), so a post
-- carries at most one tag per genus — picking a species refines that genus's tag.
-- Empty species = a genus-level tag (the prior behavior); existing rows unchanged.
alter table post_plant add column species text not null default '';

-- Rebuild post_card so each plant carries its species (null when genus-level).
-- Keystone stays genus-level (that's how the NWF data is defined).
